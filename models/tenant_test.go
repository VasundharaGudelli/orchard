...
func testTenantToManyConnectorConfigs(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, connectorConfigDBTypes, false, connectorConfigColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, connectorConfigDBTypes, false, connectorConfigColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.ConnectorConfigs().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := TenantSlice{&a}
	if err = a.L.LoadConnectorConfigs(ctx, tx, false, (*[]*Tenant)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ConnectorConfigs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ConnectorConfigs = nil
	if err = a.L.LoadConnectorConfigs(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ConnectorConfigs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTenantToManyAddOpConnectorConfigs(t *testing.T) {
	var err error
	...
	foreigners := []*ConnectorConfig{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, connectorConfigDBTypes, false, strmangle.SetComplement(connectorConfigPrimaryKeyColumns, connectorConfigColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*ConnectorConfig{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddConnectorConfigs(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ConnectorConfigs[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ConnectorConfigs[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ConnectorConfigs().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	tenantDBTypes = map[string]string{`ID`: `uuid`, `Status`: `enum.tenant_status('active','deleted','pending','expired')`, `Name`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `ViewParams`: `jsonb`, `CRMID`: `text`}
	_             = bytes.MinRead
)
