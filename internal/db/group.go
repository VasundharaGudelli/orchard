...
ORDER BY "group.name"`
	...
	fullPersonSelectClause = `JSONB_BUILD_OBJECT(
		...
		'role_ids', p.role_ids, 'crm_role_ids', p.crm_role_ids, 'is_provisioned', p.is_provisioned, 'is_synced', p.is_synced, 'status', p.status, 'photo_url', p.photo_url,
		'created_at', TO_CHAR(p.created_at, 'YYYY-MM-DD"T"HH24:MI:SS"Z"'), 'created_by', p.created_by,
		'updated_at', TO_CHAR(p.updated_at, 'YYYY-MM-DD"T"HH24:MI:SS"Z"'), 'updated_by', p.updated_by,
		'type', p."type"
	)`
...
func (svc *GroupService) GetGroupSubTree(ctx context.Context, tenantID, groupID string, maxDepth int, hydrateUsers bool, simplify bool, activeUsers bool, useManagerNames bool, excludeManagerUsers bool, viewableGroups ...string) ([]*GroupTreeNode, error) {
	...
	if hydrateUsers {
		...
		personSelect = `JSONB_BUILD_OBJECT(
			'id', p.id, 'name', p."name", 'status', p."status",	'type', p."type", 'photo_url', p.photo_url
		)`
	}
...
const (
	isCRMSyncedQuery = `WITH UserCreated AS (
		SELECT SUM(CASE WHEN created_by NOT IN ('00000000-0000-0000-0000-000000000000') THEN 1 ELSE 0 END) as user_created_count
		FROM "group"
		WHERE tenant_id = $1
		GROUP BY tenant_id
	)
